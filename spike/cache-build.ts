...
import { assetHash, buildManifest, EPISODE_BYTE_BUDGET, CHAR_TOPICS } from "@voicequest/engine";
...
const FREETALK_LINES = (CHAR_TOPICS[epData.character ?? "daiki"] ?? [])
  .map((t) => ({ text: t.question, voice: MAIN_VOICE })) // 캐릭터별 토픽 질문(캐시카우 ×N)
  .concat(["へえ、いいね！もっと聞かせて。", "なるほどね。", "うーん、そっか。", "ごめん、もう一度言ってくれる？", "うーん、その話はやめておこうか。別のことを話そう。"].map((t) => ({ text: t, voice: MAIN_VOICE }))); // 리액션은 공통(캐릭터 음색)
