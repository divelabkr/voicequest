      ...
      <View style={st.container}>
          ...
          {!entered ? (
            ...
            lines.map((l, i) => (
              <View key={i} style={[st.bubble, l.who === "you" ? st.you : st.npc]}>
                ...
                {l.furigana ? <Furigana text={l.furigana} /> : <Text style={st.bubbleText}>{l.text}</Text>}
        ...
        {entered ? (
          <Pressable style={[st.mic, busy && st.micBusy]} onPress={onMic} disabled={busy}>
            {busy ? <ActivityIndicator color="#fff" /> : <Text style={st.micText}>{rec ? "■ 멈추고 전송" : "🎤 눌러서 말하기"}</Text>}
          </Pressable>
        ) : null}
      </View>
    </View>
...
const st = StyleSheet.create({
  root: { flex: 1, backgroundColor: "#faf7f2", alignItems: "center" },
  container: { flex: 1, width: "100%", maxWidth: 600 },
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: 16, paddingTop: 56, backgroundColor: "#fff", borderBottomWidth: 0.5, borderColor: "#e5e0d8" },
  title: { fontSize: 16, fontWeight: "500", color: "#2c2c2a" },
  aff: { fontSize: 15, color: "#d4537e" },
  withdraw: { fontSize: 13, color: "#888780" },
  log: { flex: 1 },
  enter: { alignSelf: "center", marginTop: 40, paddingVertical: 14, paddingHorizontal: 32, backgroundColor: "#fff", borderRadius: 12, borderWidth: 0.5, borderColor: "#cabfa9" },
  enterText: { fontSize: 16, fontWeight: "500", color: "#2c2c2a" },
  bubble: { maxWidth: "82%", padding: 10, borderRadius: 12, marginVertical: 5 },
  npc: { alignSelf: "flex-start", backgroundColor: "#fff", borderWidth: 0.5, borderColor: "#e5e0d8" },
  you: { alignSelf: "flex-end", backgroundColor: "#e6f1fb" },
  bubbleText: { fontSize: 16, color: "#2c2c2a" },
  grade: { fontSize: 12, color: "#1d9e75", marginBottom: 2 },
  words: { marginTop: 6, paddingTop: 6, borderTopWidth: 0.5, borderColor: "#e5e0d8", gap: 1 },
  word: { fontSize: 12, color: "#5f5e5a" },
  mic: { margin: 16, padding: 16, borderRadius: 14, backgroundColor: "#185fa5", alignItems: "center" },
  micBusy: { backgroundColor: "#888" },
  micText: { fontSize: 16, fontWeight: "500", color: "#fff" },
});
